...
public class DelegatingMojosExecutionStrategy implements MojosExecutionStrategy {
    ...
    public DelegatingMojosExecutionStrategy(MavenSession session, Logger logger) {
        ...
        if (HalfLifeBuilder.BUILDER_NAME.equals(builderId)) {
            logger.info("Using mojo reordering HalfLifeBuilderExecutionStrategy");
            delegate = new HalfLifeBuilderExecutionStrategy();
        } else {
